...
void InitialProbabilities(const std::vector<uint64_t> &counts, const std::vector<uint64_t> &counts_pruned, const std::vector<Discount> &discounts, Master &master, Sorts<SuffixOrder> &primary, FixedArray<util::stream::FileBuffer> &gammas) {
  const PipelineConfig &config = master.Config();
  ...
  InitialProbabilities(config.initial_probs, discounts, master.MutableChains(), second, gamma_chains, config.counts_threshold);
...
void InterpolateProbabilities(const std::vector<uint64_t> &counts, Master &master, Sorts<SuffixOrder> &primary, FixedArray<util::stream::FileBuffer> &gammas) {
  ...
  master >> Interpolate(counts[0], ChainPositions(gamma_chains), config.counts_threshold);
...
void Pipeline(PipelineConfig config, int text_file, int out_arpa) {
  ...
  master >> AdjustCounts(counts, counts_pruned, discounts, config.counts_threshold);

  {
    FixedArray<util::stream::FileBuffer> gammas;
    Sorts<SuffixOrder> primary;
    InitialProbabilities(counts, counts_pruned, discounts, master, primary, gammas);
